...
# Modified by Hannu Henttinen
# - force rosdep to select noetic
# - Check if rosdep is already installed and do not try it again
...
echo ""
echo "#######################################################################################################################"
echo ">>> {Step 1: Configure your Ubuntu repositories and upgrade}"
echo ""
...
if [ ! -f "/etc/ros/rosdep/sources.list.d/20-default.list" ]; then
  ...
  echo "default sources list file already exists for rosdep"
fi
rosdep update --rosdistro=${name_ros_distro}
echo ""
echo "#######################################################################################################################"
